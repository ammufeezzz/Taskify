  ...
  try {
    ...
    if (!conversationId) {
      ...
      const title = firstUserMessage
        ? generateConversationTitle(
            firstUserMessage.content || firstUserMessage.text || ""
          )
        : undefined;
...
Always use the provided tools for actions.`;
    ...
    const tools = {
        ...
        inputSchema: z
          .object({
            ...
            projectId: z
              ...
              .nullish()
              .describe(
                'The project ID, key, or name (e.g., "testing" or project name) this issue belongs to (REQUIRED). Do not call this tool with project missing - always ask the user to specify which project to add the issue to.'
              ),
            workflowStateId: z
              .string()
              .nullish()
              .describe(
                'The workflow state ID or name (e.g., "Todo", "In Progress", "Done") (REQUIRED)'
              ),
            assigneeId: z
              .string()
              .nullish()
            ...
            priority: z
              ...
              .nullish()
              .describe(
                'The priority level (REQUIRED - must be one of: low, medium, high, urgent, or none). Do not call this tool with priority missing or defaulted to "none" - always ask the user to specify a priority first.'
              ),
            estimate: z
              .number()
              .nullish()
            ...
            labelIds: z
              ...
              .nullish()
              .describe(
                'Array of label names or IDs (e.g., ["Bug", "Feature", "Documentation", "Enhancement"])'
              ),
            ...
            status: z
              ...
              .optional()
              .describe(
                "Alias for workflowStateId - do not use, use workflowStateId instead"
              ),
            project: z
              .string()
              .nullish()
              .optional()
              .describe(
                "Alias for projectId - do not use, use projectId instead"
              ),
          })
          .passthrough(),
        execute: async (params) => {
          try {
            ...
            if (existingIssue) {
              return {
                success: false,
                error: `An issue with the title "${title}" already exists (Issue #${
                  existingIssue.number
                }${
                  existingIssue.project
                    ? ` in project ${existingIssue.project.name} (${existingIssue.project.key})`
                    : ""
                }). Please use a different title or update the existing issue.`,
              };
            }

            // Validate required fields FIRST - do not proceed if anything is missing
            const validation = validateIssueFields({
              title,
              workflowStateId,
              priority,
              projectId,
            ...
            if (!resolvedWorkflowStateId) {
              const availableStates = teamContext.workflowStates
                .map((ws) => ws.name)
                .join(", ");
              return {
                success: false,
            ...
            if (!resolvedProject) {
              const availableProjects =
                teamContext.projects.length > 0
                  ? teamContext.projects
                      .map((p) => `${p.name} (${p.key})`)
                      .join(", ")
                  : "No projects available";
              return {
                success: false,
                error: `The project "${projectId}" was not found. ${
                  teamContext.projects.length > 0
                    ? `Available projects: ${availableProjects}. Please specify a valid project name, key, or ID.`
                    : "Please create a project first or specify an existing project."
                }`,
              };
            }
            ...
            const issue = await createIssue(
              teamId,
              {
                ...
                labelIds:
                  resolvedLabelIds.length > 0 ? resolvedLabelIds : undefined,
              },
              userId,
      ...
      createIssues: tool({
        ...
        inputSchema: z.object({
          issues: z
              ...
              z
                .object({
                  ...
                  projectId: z
                    .string()
                    .nullish()
                    .describe("The project ID, key, or name (REQUIRED)"),
                  workflowStateId: z
                    .string()
                    .nullish()
                    .describe(
                  ...
                  priority: z
                    .enum(["none", "low", "medium", "high", "urgent"])
                    .nullish()
                    .describe("The priority level (REQUIRED)"),
                  estimate: z
                    .number()
                    .optional()
                    .describe("Story points or hours estimate"),
                  labelIds: z
                    .array(z.string())
                    .optional()
                    .describe("Array of label names or IDs"),
            ...
            .min(1)
            .describe(
              "Array of issues to create. Must contain at least 1 issue."
            ),
        }),
        execute: async ({ issues }) => {
          try {
            ...
            for (let i = 0; i < issues.length; i++) {
              ...
              if (
                !issueData.workflowStateId ||
                issueData.workflowStateId === "null" ||
                issueData.workflowStateId === "undefined"
              ) {
            ...
            if (missingFields.length > 0) {
              ...
              return {
                success: false,
                error: `I need the following information to create the issues: ${missingFields.join(
                  ", "
                )}. Please provide ${
                  missingFields.length === 1 ? "this" : "these"
                } to proceed. ${
                  teamContext.projects.length > 0 &&
                  missingFields.some((f) => f.includes("project"))
              ...
              return (async () => {
                try {
                  ...
                  const issueProjectId =
                    issueData.projectId || (issueData as any).project;
                  const issueWorkflowStateId =
                    issueData.workflowStateId || (issueData as any).status;
                  ...
                  const resolvedLabelIds = resolveLabelIds(
                    teamContext,
                    issueData.labelIds
                  );
            ...
            const successMessage =
              createdIssues.length > 0
                ? `Successfully created ${createdIssues.length} issue${
            ...
            const errorMessage =
              errors.length > 0
                ? `Failed to create ${errors.length} issue${
                    errors.length !== 1 ? "s" : ""
                  }: ${errors
                    .map((e) => `"${e.issue}" (${e.error})`)
                    .join(", ")}`
                : "";
      ...
      updateIssue: tool({
        description:
          'Update an existing issue by title or ID. Use workflow state names (like "Todo", "Backlog", "In Progress", "Done"), assignee names, and label names.',
        inputSchema: z.object({
          issueId: z
            .string()
            .optional()
          ...
          labelIds: z
            ...
            .nullish()
            .describe(
              'Array of label names or IDs (e.g., ["Bug", "Feature", "Documentation"])'
            ),
        }),
        execute: async ({
          issueId,
          ...
          try {
            ...
            if (title && !issueId) {
              ...
              if (matchingIssues.length > 1) {
                return {
                  success: false,
                  error: `Multiple issues found matching "${title}": ${matchingIssues
                    .map((i) => `#${i.number} "${i.title}"`)
      ...
      updateIssues: tool({
        description:
          'Update multiple issues at once. Accepts an array of updates. Use this when the user asks to update multiple issues (e.g., "change status of issues #1, #2, #3 to Done", "assign all issues in project X to John").',
        inputSchema: z.object({
          updates: z
            .array(
              z.object({
                ...
                workflowStateId: z
                  .string()
                  .optional()
                  .describe("The new workflow state ID or name"),
                assigneeId: z
                  .string()
                  .nullish()
                  .describe("The user ID or name to assign this issue to"),
            ...
            .min(1)
            .describe(
              "Array of issue updates. Must contain at least 1 update."
            ),
        }),
        execute: async ({ updates }) => {
          try {
            if (!updates || updates.length === 0) {
              return {
                success: false,
                error: "At least one issue update is required",
              };
            }
              ...
              try {
                ...
                if (updateData.title && !updateData.issueId) {
                  ...
                  const matchingIssues = issues.filter((issue: any) =>
                    issue.title
                      .toLowerCase()
                      .includes((updateData.title || "").toLowerCase())
                  );

                  if (matchingIssues.length === 0) {
                    errors.push({
                      issue: updateData.title,
                  ...
                  if (matchingIssues.length > 1) {
                    errors.push({
                      issue: updateData.title,
                      error: `Multiple issues found matching "${updateData.title}"`,
                    });
                    continue;
                  }
                ...
                if (!resolvedIssueId) {
                  errors.push({
                    issue: updateData.title || "unknown",
                    error: "Either issueId or title must be provided",
                  });
                  continue;
                }
                ...
                if (
                  ...
                  updateData.assigneeId !== "unassigned" &&
                ...
                const issue = await updateIssue(teamId, resolvedIssueId, {
                  ...
                  ...(updateData.estimate !== undefined && {
                    estimate: updateData.estimate || null,
                  }),
                  ...(resolvedLabelIds && { labelIds: resolvedLabelIds }),
                });
                ...
                if (!issue) {
                  errors.push({
                    issue: updateData.title || updateData.issueId || "unknown",
            ...
            const successMessage =
              updatedIssues.length > 0
                ? `Successfully updated ${updatedIssues.length} issue${
            ...
            const errorMessage =
              errors.length > 0
                ? `Failed to update ${errors.length} issue${
                    errors.length !== 1 ? "s" : ""
                  }: ${errors
                    .map((e) => `"${e.issue}" (${e.error})`)
                    .join(", ")}`
      ...
      deleteIssues: tool({
        description:
          'Delete multiple issues at once. Accepts an array of issue IDs or titles. Use this when the user asks to delete multiple issues (e.g., "delete issues #1, #2, #3", "delete all issues in project X").',
        inputSchema: z.object({
          issues: z
            .array(
              z.object({
                ...
                title: z
                  ...
                  .optional()
                  .describe(
                    "The title of the issue to find and delete (if issueId not provided)"
                  ),
              })
            )
            .min(1)
            .describe(
              "Array of issues to delete. Must contain at least 1 issue."
            ),
        }),
        execute: async ({ issues }) => {
          try {
            ...
            for (const issueData of issues) {
              try {
                ...
                if (issueData.title && !issueData.issueId) {
                  ...
                  const matchingIssues = allIssues.filter((issue: any) =>
                    issue.title
                      .toLowerCase()
                      .includes((issueData.title || "").toLowerCase())
                  );
                  ...
                  if (matchingIssues.length > 1) {
                    errors.push({
                      issue: issueData.title,
                      error: `Multiple issues found matching "${issueData.title}"`,
                    });
                    continue;
                  }
                ...
                errors.push({
                  issue: issueData.title || issueData.issueId || "unknown",
                  error: error.message || "Failed to delete issue",
                });
              }
            }
            ...
            const successMessage =
              deletedIssues.length > 0
                ? `Successfully deleted ${deletedIssues.length} issue${
                    deletedIssues.length !== 1 ? "s" : ""
                  }: ${deletedIssues
                    .map((i) => `#${i.number} "${i.title}"`)
                    .join(", ")}`
                : "";
            ...
            const errorMessage =
              errors.length > 0
                ? `Failed to delete ${errors.length} issue${
                    errors.length !== 1 ? "s" : ""
                  }: ${errors
                    .map((e) => `"${e.issue}" (${e.error})`)
                    .join(", ")}`
      ...
      createProject: tool({
        description:
          'Create a single new project. For creating multiple projects at once (2+), use createProjects instead. The name and key are REQUIRED. If any required information is missing (name or key), ask the user for it. The color parameter is optional and defaults to #6366f1 if not provided. The status parameter is optional and defaults to "active" if not provided.',
        inputSchema: z.object({
          name: z
            .string()
            .nullish()
            .describe(
              "The name of the project (REQUIRED). Do not call this tool with name missing - always ask the user to specify the project name first."
            ),
          description: z.string().optional(),
          key: z
            .string()
            .nullish()
            .describe(
              "A 3-letter project identifier (REQUIRED). Do not call this tool with key missing - always ask the user to specify a 3-letter project key first."
            ),
        ...
        execute: async ({
          ...
          try {
            ...
            if (missingFields.length > 0) {
              ...
              if (
                missingFields.includes("key (3-letter project identifier)") &&
                missingFields.length === 1
              ) {
                return {
                  success: false,
                  error:
                    'To create a project, I need a 3-letter project identifier (key). Please provide a short 3-letter code for this project (e.g., "WEB", "API", "MOB").',
                };
              }
              ...
              if (
                ...
                return {
                  success: false,
                  error:
                    "To create a project, I need the project name. Please specify what you would like to name this project.",
                };
              }
              ...
              return {
                success: false,
                error: `To create a project, I need both: (1) the project name, and (2) a 3-letter project identifier (key). Please provide ${
                  missingFields.length === 1 ? "this" : "these"
                } to proceed.`,
              };
            }
            ...
            if (key && key.length !== 3) {
              return {
                success: false,
                error: `The project key must be exactly 3 letters. You provided "${key}" which is ${
                  key.length
                } character${
                  key.length !== 1 ? "s" : ""
                }. Please provide a 3-letter key (e.g., "WEB", "API", "MOB").`,
              };
            }
      ...
      createProjects: tool({
        description:
          'Create multiple projects at once. Accepts an array of project objects. Each project needs: name and key (3-letter identifier). Use this when the user asks to create multiple projects (e.g., "create 5 projects", "create projects for frontend, backend, and API").',
        inputSchema: z.object({
          projects: z
            .array(
              z.object({
                name: z
                  .string()
                  .nullish()
                  .describe("The name of the project (REQUIRED)"),
                description: z.string().nullish(),
                key: z
                  .string()
                  .nullish()
                  .describe("A 3-letter project identifier (REQUIRED)"),
                color: z.string().optional(),
                icon: z.string().optional(),
                leadId: z.string().optional(),
                status: z
                  ...
                  .nullish()
                  .describe(
                    "The status of the project (optional, defaults to active)"
                  ),
              })
            )
            .min(1)
            .describe(
              "Array of projects to create. Must contain at least 1 project."
            ),
        }),
          ...
          try {
            ...
            for (let i = 0; i < projects.length; i++) {
              ...
              if (
                ...
                missingFields.push(
                  `Project ${i + 1}: key (3-letter identifier)`
                );
              }
            }
            ...
            if (missingFields.length > 0) {
              return {
                success: false,
                error: `I need the following information to create the projects: ${missingFields.join(
                  ", "
                )}. Please provide ${
                  missingFields.length === 1 ? "this" : "these"
                } to proceed.`,
              ...
              try {
                ...
                if (projectKey.length !== 3) {
                  errors.push({
                    ...
                    error: `Key "${projectKey}" must be exactly 3 letters`,
                  });
                  continue;
                }

                // Check if key already exists
                const existingProject = teamContext.projects.find(
                  (p) => p.key.toLowerCase() === projectKey.toLowerCase()
                );
                if (existingProject) {
                  errors.push({
                    project: projectData.name || "Unknown",
                    error: `Project key "${projectKey}" already exists`,
                  });
                  continue;
                }
                ...
                const project = await createProject(teamId, {
                  ...
                  lead: projectData.leadId
                    ? teamContext.members.find(
                        (m) => m.userId === projectData.leadId
                      )?.userName
                    : undefined,
                });
            ...
            const successMessage =
              createdProjects.length > 0
                ? `Successfully created ${createdProjects.length} project${
            ...
            const errorMessage =
              errors.length > 0
                ? `Failed to create ${errors.length} project${
                    errors.length !== 1 ? "s" : ""
                  }: ${errors
          ...
          try {
            ...
            if (name && !projectId) {
              ...
              if (matchingProjects.length > 1) {
                return {
                  success: false,
                  error: `Multiple projects found matching "${name}": ${matchingProjects
                    .map((p) => `${p.name} (${p.key})`)
                    .join(", ")}. Please be more specific.`,
                };
              }
      ...
      inviteTeamMember: tool({
        description:
          "Invite a single new team member via email. For inviting multiple team members at once (2+), use inviteTeamMembers instead.",
        inputSchema: z.object({
      ...
      inviteTeamMembers: tool({
        description:
          'Invite multiple team members at once. Accepts an array of email addresses with optional roles. Use this when the user asks to invite multiple people (e.g., "invite 10 team members", "invite john@example.com, jane@example.com, and bob@example.com").',
        inputSchema: z.object({
          invitations: z
            .array(
              z.object({
                email: z
                  ...
                  .min(5)
                  .describe(
                    "The email address to invite (REQUIRED). Must be a valid email format."
                  ),
                role: z
                  .string()
                  .optional()
                  .default("developer")
                  .describe(
                    "The role for the member (developer, admin, viewer)"
                  ),
              })
            )
            .min(1)
            .describe(
              "Array of invitations to send. Must contain at least 1 invitation."
            ),
        }),
            ...
            for (const invitationData of invitations) {
              try {
                ...
                try {
                  ...
                  console.error(
                    `Error sending invitation email to ${invitationData.email}:`,
                    emailError
                  );
            ...
            const successMessage =
              sentInvitations.length > 0
                ? `Successfully sent ${sentInvitations.length} invitation${
                  ...
                  } to: ${sentInvitations.map((i) => i.email).join(", ")}`
                : "";

            const errorMessage =
              errors.length > 0
                ? `Failed to send ${errors.length} invitation${
                    errors.length !== 1 ? "s" : ""
      ...
      deleteProject: tool({
        description:
          "Delete a project by name or ID. This will permanently remove the project and all associated issues.",
        inputSchema: z.object({
          projectId: z
            .string()
            .optional()
          ...
          name: z
            ...
            .optional()
            .describe(
              "The name of the project to find and delete (if projectId not provided)"
            ),
        }),
              ...
              if (matchingProjects.length > 1) {
                return {
                  ...
                  error: `Multiple projects found matching "${name}": ${matchingProjects
                    ...
                    .join(
                      ", "
                    )}. Please be more specific with the project name or use the project ID.`,
                };
              }
      ...
      addProjectMember: tool({
        description:
          "Add a team member to a project. The member must already be a team member. Use project name or ID, and member name or email.",
        inputSchema: z.object({
          projectId: z.string().optional().describe("The ID of the project"),
          projectName: z
            .string()
            .optional()
            .describe("The name of the project (if projectId not provided)"),
          ...
          try {
            ...
            if (projectName && !projectId) {
              ...
              if (matchingProjects.length > 1) {
                return {
                  success: false,
                  error: `Multiple projects found matching "${projectName}": ${matchingProjects
                    .map((p) => `${p.name} (${p.key})`)
                    .join(", ")}. Please be more specific.`,
                };
              }
            ...
            if (userId) {
              ...
              teamMember = teamContext.members.find(
                (m) => m.userEmail.toLowerCase() === userEmail.toLowerCase()
              );
            } else if (userName) {
              teamMember = teamContext.members.find((m) =>
                m.userName.toLowerCase().includes(userName.toLowerCase())
              );
            }

            if (!teamMember) {
              const availableMembers = teamContext.members
                .map((m) => `${m.userName} (${m.userEmail})`)
                .join(", ");
              return {
                success: false,
                error: `Team member not found. Available team members: ${
                  availableMembers || "No team members found"
                }`,
              };
            }
            ...
            if (existingMember) {
              return {
                success: false,
                error: `${teamMember.userName} is already a member of this project`,
              };
            }
      ...
      removeProjectMember: tool({
        description:
          "Remove a team member from a project. Use project name or ID, and member name or email.",
        inputSchema: z.object({
          projectId: z.string().optional().describe("The ID of the project"),
          projectName: z
            .string()
          ...
          userName: z
            .string()
            .optional()
            .describe("The name of the user to remove"),
        }),
        execute: async ({
          projectId,
          projectName,
      ...
      listProjectMembers: tool({
        description:
          "List all members of a specific project. Use project name or ID.",
        inputSchema: z.object({
          projectId: z.string().optional().describe("The ID of the project"),
          projectName: z
            .string()
        ...
        execute: async ({ projectId, projectName }) => {
          try {
            ...
            return {
              ...
              message:
                members.length === 0
                  ...
                  : `Project "${project.name}" has ${members.length} member${
                      members.length !== 1 ? "s" : ""
            ...
            return {
              success: false,
              error: error.message || "Failed to list project members",
            };
          }
        },
      }),
      ...
      revokeInvitation: tool({
        description:
          "Revoke or cancel a pending team invitation by email or invitation ID",
        inputSchema: z.object({
          invitationId: z
            .string()
            .optional()
            .describe("The ID of the invitation to revoke"),
          email: z
            .string()
            .optional()
            .describe(
              "The email address of the pending invitation to revoke (if invitationId not provided)"
            ),
        }),
          ...
          try {
            ...
            if (email && !invitationId) {
              ...
              if (!invitation) {
                return {
                  success: false,
                  error: `No pending invitation found for email "${email}"`,
                };
              }
              ...
              if (invitation.status !== "pending") {
                return {
                  success: false,
                  error: `Invitation for "${email}" is not pending (status: ${invitation.status})`,
                };
              }
            ...
            if (!resolvedInvitationId) {
              return {
                success: false,
                error: "Either invitationId or email must be provided",
              };
            }
      ...
      removeTeamMember: tool({
        description:
          "Remove a team member from the workspace by user ID, email, or name. Only admins can remove team members.",
        inputSchema: z.object({
          memberId: z
            .string()
            .optional()
            .describe("The team member ID to remove"),
          userId: z
            .string()
            .optional()
            .describe(
              "The user ID of the member to remove (if memberId not provided)"
            ),
          email: z
            .string()
            .optional()
            .describe(
              "The email address of the member to remove (if memberId or userId not provided)"
            ),
          name: z
            .string()
            .optional()
            .describe(
              "The name of the member to remove (if memberId, userId, or email not provided)"
            ),
        }),
          ...
          try {
            ...
            if (!resolvedMemberId) {
              return {
                success: false,
                error:
                  "Either memberId, userId, email, or name must be provided",
              };
            }
        ...
        if (msg.toolCalls && Array.isArray(msg.toolCalls)) {
          msg.toolCalls.forEach((tc: any) => {
            parts.push({
              ...
              toolCallId:
                tc.toolCallId || tc.id || `tc-${Date.now()}-${Math.random()}`,
